...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await updateTransactions('sendmoney', 'topup', ids, updates);
...
export async function DELETE(request: Request) {
  try {
    ...
    const result = await deleteTransaction('sendmoney', 'topup', id);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof TransactionActionError) return NextResponse.json({ error: err.message }, { status: err.status });
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createTransaction('sendmoney', 'topup', input);
