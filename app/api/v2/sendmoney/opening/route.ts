...
import { getSendMoneyOpeningPgRows } from '@/app/lib/services/openingPageService';
...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await updateOpeningAgents('sendmoney', agentCodes, updates);
...
export async function DELETE(request: Request) {
  try {
    ...
    const result = await deleteOpeningAgent('sendmoney', agentCode);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof OpeningActionError) return NextResponse.json({ error: err.message }, { status: err.status });
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createOpeningAgent('sendmoney', input);
