...
// Single Edit and Bulk Edit share this one endpoint: ids is [oneId] for
// Single Edit, or the full selection for Bulk Edit — both resolve to the
// same one-transaction, all-or-nothing service call.
export async function PATCH(request: Request) {
  try {
    ...
    const result = await updateTransactions('cashout', 'settlement', ids, updates);
...
export async function DELETE(request: Request) {
  try {
    ...
    const result = await deleteTransaction('cashout', 'settlement', id);
    return NextResponse.json(result);
  } catch (err) {
    if (err instanceof TransactionActionError) return NextResponse.json({ error: err.message }, { status: err.status });
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createTransaction('cashout', 'settlement', input);
