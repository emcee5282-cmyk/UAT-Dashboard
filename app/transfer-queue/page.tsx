...
// LOCALHOST-ONLY, page-scoped data-source override — same pattern as
// app/sendmoney/balances/page.tsx's own isPostgresSourceEnabled(). Explicit
// opt-in only: any value other than the literal 'postgres' keeps this page
// on Google Sheets, its always-safe default.
...
// Compact sizing (matches Wallet Status's density): font/padding/row
// height scaled down, which shrinks every header's own natural width
...
// Remarks — verified safe against the FULL live dataset (1,888 rows):
// longest real Shop Name needs 172px text (194px with padding) against its
// 198px column; longest Company Balance needs 59px text (92px with
...
function renderCell(row: QueueRow, key: ColumnKey, searchTerm: string) {
  ...
  switch (key) {
    case 'brand':
      return <td key={key} className={`${leftBase} text-foreground`}>{highlightMatch(row.brand, searchTerm)}</td>;
    case 'shopName':
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-background font-[Inter,sans-serif] text-foreground transition-colors duration-300 dark:bg-[#0A0C11]">
      ...
      {/* px-4 md:px-[28px] + the inner mx-auto max-w-[1400px] wrapper (no
          ...
          pills) now that there's no KPI band between the header and this
          section to account for separately. */}
