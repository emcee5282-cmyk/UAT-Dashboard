...
export default function Sidebar() {
    ...
    <>
      <div className="flex items-center justify-between border-b border-white/10 px-5 py-6">
        <div className="flex items-center gap-3">
          <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-500/15 ring-1 ring-indigo-400/30">
            <Sparkles size={16} className="text-indigo-300" />
          </div>
          <div>
            <p className="text-sm font-semibold text-white">Operations</p>
            <p className="text-[10px] uppercase tracking-[0.24em] text-[#a0a0a0]">Overview</p>
          </div>
        </div>
        <button onClick={() => setMobileOpen(false)} className="text-[#a0a0a0] hover:text-white md:hidden">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="border-t border-white/10 px-5 py-4">
        <div className="mb-3 rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-[11px] text-[#a0a0a0]">
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-full w-64 flex-col border-r border-[#3a3a3d] bg-[#1c1c1e] text-white transition-transform duration-300 md:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <NavContent />
      </aside>

      <aside className="hidden h-screen w-64 shrink-0 border-r border-[#3a3a3d] bg-[#1c1c1e] text-white md:flex md:flex-col">
        <NavContent />
      </aside>
    </>
  );
