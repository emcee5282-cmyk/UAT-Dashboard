...
import { Search, ChevronUp, ChevronDown, ChevronsUpDown, Columns3, Download, PlusCircle, RefreshCw, MoreVertical, Copy, Pencil, Eye, Trash2, Inbox, Tag, User, Wallet as WalletIcon, FilterX, Upload, Plus, CheckSquare, X } from 'lucide-react';
...
// (app/daily-txn-entry/page.tsx) exactly, same as Cashout Top Up
// (app/topup/page.tsx): Manrope for UI text/labels/body copy, Space Grotesk
// for anything that reads as data/a number (applied via the
// `.topup-page .tabular-nums` rule below). Every other page keeps Inter.
...
// kept as the extension point for a future label override. 'SH' shows
// as-is (no override) per explicit instruction, matching the Wallet
...
export default function SendMoneyTopUpPage() {
  ...
  // Compact stat bar, matching Daily Txn Entry's card chrome for tone only
  // (font/color tokens) — not its tile layout. One hero stat (Total Amount
  // + full-figure subtitle + a trend chip comparing to yesterday) plus
  // Today's Count and Yesterday's Count as two smaller stats. Yesterday's
  ...
  return (
    <div className={`topup-page h-screen w-full flex flex-col overflow-hidden bg-background text-foreground transition-colors duration-300 dark:bg-[#0A0C11] ${manrope.variable} ${spaceGrotesk.variable}`}>
      ...
      {/* px-4 md:px-[28px] + the inner mx-auto max-w-[1400px] wrapper (no
          ...
          against that page. Matches Cashout Top Up (app/topup/page.tsx)
          exactly. */}
      <main className="flex-1 flex flex-col overflow-hidden px-4 pb-6 md:px-[28px] md:pb-8">
        ...
        {!error && (
          ...
          // Matches Cashout Top Up (app/topup/page.tsx) exactly.
          <div className="flex-1 flex flex-col min-h-0 rounded-lg border border-border bg-white px-[22px] py-4 overflow-hidden dark:bg-[#12151D]">
            {/* Compact horizontal stat bar — replaces the earlier bordered
                ...
                hairline dividers instead of separate boxes. Matches Cashout
                Top Up (app/topup/page.tsx) exactly. */}
            ...
            <div className="shrink-0 flex flex-nowrap items-center overflow-x-auto border-b border-border px-[13px] py-[10px]">
              {loading ? (
