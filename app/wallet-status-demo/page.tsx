...
// DEMO ONLY: toolbar buttons scaled to 80% (40px -> 32px), matching the
// table's own compact sizing per explicit follow-up request.
const ICON_BUTTON =
  'flex h-8 w-8 xl:w-auto shrink-0 items-center justify-center xl:justify-start gap-[5px] rounded-[10px] border border-[#E2E8F0] bg-white px-0 xl:px-[10px] text-[11px] font-medium text-[#475569] transition-[color,background-color,border-color,box-shadow,transform] duration-150 ease-[var(--ease-out-strong)] hover:border-[#2563EB] hover:bg-[#F1F5F9] hover:ring-2 hover:ring-[#2563EB]/20 active:scale-[0.97] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2563EB] dark:border-[#262B38] dark:bg-[#12151D] dark:text-[#9CA3AF] dark:hover:bg-white/5';
...
const DEFAULT_COLUMNS: ColumnDef[] = [
  ...
  // DEMO ONLY: fully removed per explicit repeat request — hiding it by
  // default (visible: false, still hideable) wasn't enough since it could
  // still be toggled back on via the Columns dropdown or a stale saved
  // preference. Dropped from DEFAULT_COLUMNS entirely so it can never
  // reappear regardless of prior state.
  ...
  // DEMO ONLY: always left-aligned per explicit instruction (header and
  // data both — see RemarksCell's "No Remarks" placeholder fix too,
  // which was inconsistently centered while a real remark was left).
...
// DEMO ONLY: distinct key from the real page's 'walletStatusColumnVisibility'
// — otherwise a saved preference from either page silently overrides the
// other's defaults (this is why SDP kept reappearing despite being hidden
// by default below).
const COLUMN_VISIBILITY_STORAGE_KEY = 'walletStatusDemoColumnVisibility';
...
// DEMO ONLY: every value below is the real page's value scaled to 80%
// (20% smaller), per explicit request to shrink the demo table's row/col
// sizing — font, padding, row height, badge chrome, all scaled together
// so nothing drifts out of proportion.
// DEMO ONLY (round 3): +1px on every table font size per explicit
// follow-up request — row height/padding unchanged this round.
...
// DEMO ONLY (round 2): column WIDTH tightened another 20% on top of the
// round-1 sizing — padding/chrome shrunk so columns hug the (already
// smaller) text instead of leaving old-font-sized whitespace around it.
// DEMO ONLY (round 4): trimmed further to eliminate 100%-zoom overflow —
// verified against the FULL live dataset (4,322 rows) that the longest
// real Shop Name needs 172px of text alone, so Shop Name itself had zero
// slack to give; these chrome/padding/reserve constants (pure UI gutter,
// not text) absorbed the savings instead.
// px-[8px] cell padding (was px-[10px]) = 8px each side = 16px total.
...
// Brand badge chrome beyond its text: px-[5px] (5px) each side + 1px
// border each side.
const BRAND_BADGE_CHROME_PX = 12;
...
const REMARKS_COLUMN_WIDTH_PX = 115;
...
// DEMO ONLY: per-column min/max px bounds. `min` is a floor only (never
// causes truncation). `max` is a generous safety ceiling for a genuinely
...
// checkbox/shop columns) rather than clipping text, per Round 1.
// Round 2: `min` floors tightened another 20% (columns hug the
// already-smaller text instead of padding out to the round-1 floor);
// `max` raised to a safety ceiling instead of an active clamp, per
// explicit instruction that no real value may ever show "...".
const COLUMN_LIMITS: Partial<Record<ColumnKey, { min: number; max: number }>> = {
  brand: { min: 45, max: 150 },
  ...
  // space without growing unbounded on its own.
  remarks: { min: 115, max: 260 },
};
...
function computeColumnWidthsPx(rows: WalletStatusRow[], columns: ColumnDef[]): Partial<Record<ColumnKey, number>> {
  ...
  for (const col of columns) {
    ...
    // DEMO ONLY: `center`-aligned headers reserve the icon group's width
    // TWICE (once as the real icon, once as its invisible mirror-spacer —
    // see the header button JSX), since true text-only centering requires
    // it; every other alignment only needs it once.
    ...
    // DEMO ONLY: clamp into the column's configured min/max range.
...
// DEMO ONLY: Daily Limit/Available Limit info tooltips removed per
// explicit request — also drops their header icon-reserve requirement
// (see COLUMNS_WITH_INFO_ICON below), reclaiming real width.
...
function BrandBadge({ children }: { children: string }) {
  return (
    <span className={`inline-flex h-[22px] items-center rounded-[999px] border px-[5px] text-[11px] font-semibold transition-[filter] duration-150 hover:brightness-95 dark:hover:brightness-110 ${brandBadgeClasses(children)}`}>
      {children}
    </span>
  );
}
...
export default function WalletStatus() {
  ...
  // Checkbox column is a fixed 28px outside colWidthsPx (its own hardcoded
  // `w-[28px]` th below, 44px scaled to 64% across both demo rounds).
  const CHECKBOX_COLUMN_WIDTH_PX = 28;
  // DEMO ONLY: proportional grow-to-fill (every flexible column scales up
  // together) for everything except the columns explicitly pinned at
  // their bare minimum natural width: Shop Name (per earlier correction),
  // Wallet Status (per this round — reduced to minimum so Remarks gets
  // more of the shared leftover instead), and Action (icon-only, always
  // fixed). Remarks itself stays in the flexible pool alongside Brand/
  // Leader/Type/etc.
  const scaledColWidthsPx = useMemo(() => {
    ...
    // DEMO ONLY: canvas-measured text width (colWidthsPx) drifts a few px
    // from actual rendered DOM width per column — harmless on its own,
    // but this subtraction has no self-correcting ratio the way the
    ...
    // guarantee against that — explicit priority per instruction that
    // there must be zero overflow at 100% zoom.
  ...
  function renderCell(row: WalletStatusRow, key: ColumnKey, colWidthsPx?: Partial<Record<ColumnKey, number>>) {
    // DEMO ONLY: font/padding scaled to 80% of the real page's values
    // (round 2: horizontal padding tightened another 20%; round 3: +1px
    // on font size — row height/vertical padding unchanged since round 1).
    // DEMO ONLY: vertical-align middle (was top) — per explicit request,
    // row content should sit centered within the row, not flush at the
    // top edge (badges/text have different intrinsic heights, so
    // align-top made them look visually uneven row to row).
    ...
    // Limit) per the demo's alignment spec. Right padding is NOT symmetric
    // with the left — both of these columns' headers reserve a fixed
    // ~32px gutter on their right edge for the sort+info icon group (the
    // header label's own right edge sits at that inset, not the cell's
    // true edge, per the earlier "label position, not the icon" fix), so
    // the data's right edge must match that SAME inset, not the cell's
    // true right edge, or the header word and the numbers below it drift
    // out of alignment — measured live: was a consistent 32px gap.
    ...
    switch (key) {
      case 'brand':
        return <td key={key} style={cellStyle} className={shopBase}><BrandBadge>{row.brand}</BrandBadge></td>;
      case 'shopName': {
        // DEMO ONLY: pinned left (after the checkbox column) so it stays
        // visible while the rest of the table scrolls horizontally.
        const stickyBg = selectedIds.has(row._id) ? 'bg-[#EFF6FF] dark:bg-[#1e2a3d]' : 'bg-white dark:bg-[#0A0C11]';
        return (
      ...
      case 'balanceLimit':
        return (
          <td key={key} style={cellStyle} className={`${rightBase} tabular-nums text-foreground`}>
      ...
      case 'remarks':
        // DEMO ONLY: uses the shared `cellStyle` (from scaledColWidthsPx)
        // now, not a hardcoded fixed width — Remarks is the column that
        // absorbs leftover container space (see scaledColWidthsPx), so its
        // rendered width must track that dynamic value, floored at
        // REMARKS_COLUMN_WIDTH_PX. The remark TEXT itself still never
        // grows/wraps with content (RemarksCell's own truncate+tooltip is
        // unaffected) — only the column's own width is now dynamic.
        return (
          <td
            key={key}
            style={cellStyle}
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-background font-[Inter,sans-serif] text-foreground transition-colors duration-300 dark:bg-[#0A0C11]">
      ...
      <SettlementHeader icon={Flag} title="Wallet Status (Demo)" isRefreshing={spinning} onRefresh={fetchData} />

      <main className="flex-1 flex flex-col overflow-hidden px-6 pb-6 pt-4">
        {error && <ConnectionErrorState error={error} onRetry={fetchData} />}

        {!error && (
          <WalletLimitUsedPanel rows={rows} />
        )}
        ...
        {!error && (
          <DataTable>
            ...
            <div className="flex shrink-0 flex-nowrap items-center overflow-x-auto border-b border-[#E5E7EB] px-[13px] py-[10px] dark:border-[#262B38]">
              ...
              <div className="flex h-8 flex-1 min-w-[200px] items-center gap-[6px] rounded-full border border-[#E5E7EB] bg-white px-[13px] transition-colors focus-within:border-[#2563EB] focus-within:ring-2 focus-within:ring-[#2563EB]/20 dark:border-[#262B38] dark:bg-[#12151D]">
                {loading ? (
                      ...
                      {visibleColumns.map((col) => (
                        ...
                        >
                          {loading ? (
                            ...
                            >
                              {/* DEMO ONLY: icon stays in normal flow (never
                                  position:absolute) so it can never be
                                  clipped by the header cell's overflow-hidden
                                  on a tight column — that's what made the
                                  chevron disappear (round 3). Round 6, per
                                  explicit instruction: `center` gets its
                                  invisible mirror-spacer back — centering
                                  must be based on the header WORD's own
                                  character length only, not shifted by the
                                  chevron/info icon's reserved space. This
                                  does cost a little extra column width
                                  (icon reserved twice) versus the flush
                                  technique `right` still uses below, but
                                  that's the explicit tradeoff asked for. */}
                  ...
                  <tbody>
                    {loading ? (
                      Array.from({ length: 18 }).map((_, rowIndex) => (
                        <tr key={rowIndex}>
                          <td className="px-[8px] py-[11px] sticky left-0 z-[40] bg-white dark:bg-[#0A0C11]" />
                          {visibleColumns.map((col) => {
                            ...
                            return (
                              <td
                                ...
                                className={`px-[8px] py-[11px] ${col.key === 'walletStatusAction' ? 'sticky right-0 z-[40] bg-white dark:bg-[#0A0C11]' : ''} ${col.key === 'shopName' ? 'sticky left-[28px] z-[40] bg-white dark:bg-[#0A0C11]' : ''}`}
                              >
                                <div className={`dt-skeleton h-[8px] rounded-md ${width}`} />
                              </td>
                            );
                          })}
                        </tr>
                      ...
                      >
                        <td
                          className={`px-[8px] py-[11px] text-center align-middle sticky left-0 z-[40] ${selectedIds.has(row._id) ? 'bg-[#EFF6FF] dark:bg-[#1e2a3d]' : 'bg-white dark:bg-[#0A0C11]'}`}
                          onClick={(event) => event.stopPropagation()}
                        >
                          <input
                            type="checkbox"
              ...
              <div className="flex flex-col gap-2 p-3">
                {loading ? (
                  Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="rounded-xl border border-border bg-white p-3.5 dark:bg-[#12151D]">
                  ...
                  pagedRows.map((row) => {
                    ...
                    return (
                      <div
                        key={row.key}
                        className={`relative rounded-xl border p-3.5 pr-9 transition-[background-color,border-color] duration-150 ease-out dark:bg-[#12151D] ${
                          isSelected
                      ...
                      >
                        ...
                        {(showShop || showBrand || showWalletType) && (
                        ...
                        {(showBalanceLimit || showAvailableLimit || showSdp) && (
                          <div className={`grid grid-cols-2 gap-2 ${(showShop || showBrand || showWalletType || showLeader) ? 'mt-2.5 border-t border-border pt-2.5' : ''}`}>
                            {showBalanceLimit && (
                              <div>
                        ...
                        {(showDeposit || showWithdrawal || showSchedule) && (
                          <div className={`flex flex-wrap items-center gap-2 ${(showShop || showBrand || showWalletType || showLeader || showBalanceLimit || showAvailableLimit || showSdp) ? 'mt-2.5 border-t border-border pt-2.5' : ''}`}>
                            {showDeposit && (
                              <div>
                        ...
                        {showWalletStatus && (
                          <div className={`flex items-center gap-1.5 ${(showShop || showBrand || showWalletType || showLeader || showBalanceLimit || showAvailableLimit || showSdp || showDeposit || showWithdrawal || showSchedule) ? 'mt-2.5 border-t border-border pt-2.5' : ''}`}>
