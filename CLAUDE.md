...
- **/sendmoney/balances (app/sendmoney/balances/page.tsx)** duplicates Cashout's Agent Balance page (app/agentbal/page.tsx) the same way Opening Balance duplicates Cashout's Opening page — same columns/layout/filters/buttons, teal accent instead of indigo. Data sources: master roster + SDP/Opening/Leader from "Opening AG" cols L-O (same sheet /api/opening already fetches for Cashout, index shift +11), wallet activity (Total DP/WD, Balance, Login, Account Status, Group/Bank) from "SSP PS BalanceLimit" via /api/sendmoney/balances. Adaptations from Cashout's version (not literal copies): no SDP VS Balance leader-exclusion list (Cashout's 19-name list doesn't apply to Send Money's own roster — add one here if a Send Money exclusion policy is ever defined); Wallet Type match strings use Send Money's own Bank-field suffix ("NAGADC"/"ROCKETC"/"UPAYC" vs Cashout's "NAGAD"/"ROCKET"/"UPAY" — confirmed by sampling, not assumed); brand codes include Send Money's own 'SH' brand. **Known gap: Total Top Up and Total Settlement are hardcoded to 0** — the "Stlm Top Up" sheet's second block of columns (16-28) looked like a mirrored Send Money section but sampling proved its agent codes belong to Cashout's own roster, so it was never wired in. Company Balance is therefore Opening + Total DP − Total WD only, missing the Top Up/Settlement terms Cashout's version has. Don't add a Top Up/Settlement source for this page without verifying it's genuinely Send Money data first (same sampling method: check whether the codes/wallet names in it match Send Money's roster from "Opening AG" cols L-O, not Cashout's cols A-D).
